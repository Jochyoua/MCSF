...
public class Utils {
    ...
    public void reload() {
        if (supported("mysql")) {
            ...
            try (Connection connection = connector.getConnection()) {
                ...
                rs = connection.prepareStatement("SELECT * FROM users;").executeQuery();
                while (rs.next()) {
                ...
                rs = connection.prepareStatement("SELECT * FROM whitelist;").executeQuery();
                while (rs.next()) {
    ...
    public String prepare(CommandSender player, String message) {
        message = ChatColor.translateAlternateColorCodes('&', message.replaceAll("(?i)\\{prefix}|(?i)%prefix%", Objects.requireNonNull(plugin.getLanguage().getString("variables.prefix")))
                ...
                .replaceAll("(?i)\\{wordcount}|(?i)%wordcount%", Integer.toString(plugin.getConfig().getStringList("swears").size())));
