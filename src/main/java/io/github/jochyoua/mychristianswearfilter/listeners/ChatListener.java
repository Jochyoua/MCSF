        ...
        if (use || !manager.supported("ProtocolLib")) {
            if (!manager.getGlobalSwears().isEmpty()) {
                if (plugin.getConfig().getBoolean("settings.filtering.global blacklist.enabled")) {
                    new_message = manager.clean(old_message, false, manager.reloadPattern(Types.Filters.GLOBAL), Types.Filters.PLAYERS);
            ...
            for (Player player : e.getRecipients()) {
                if (new User(manager, player.getUniqueId()).status() || plugin.getConfig().getBoolean("settings.filtering.force")) {
                    player.sendMessage(String.format(e.getFormat(), e.getPlayer().getDisplayName(), manager.clean(new_message, false, manager.reloadPattern(Types.Filters.BOTH), Types.Filters.PLAYERS)));
                    remove.add(player);
                }
            }
