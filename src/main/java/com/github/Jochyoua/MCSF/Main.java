    ...
    public void onEnable() {
        ...
        if (!getConfig().getBoolean("settings.only_filter_players")) {
            try {
                ...
                plugin.getLogger().log(Level.SEVERE, plugin.getConfig().getString("variables.failure")
                        .replace("%message%",
                                plugin.getConfig().getString("variables.error.execute_failure")
                                        .replace("%feature%", "Chat Filtering (ProtocolLib)")), e);
                plugin.getLogger().log(Level.INFO, plugin.getConfig().getString("variables.failure").replace("%message%", plugin.getConfig().getString("variables.error.execute_failure_link")));
            }
        }
